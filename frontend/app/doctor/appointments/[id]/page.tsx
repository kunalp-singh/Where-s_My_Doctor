  ...
  const handleCompleteVisit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const interval = setInterval(async () => {
        try {
          retries++;
          const updatedDetail = await getVisitDetail(appointmentId);
          if (
            updatedDetail.aiPostVisitSummaryStatus === "summary_ready" ||
            updatedDetail.aiPostVisitSummaryStatus === "summary_failed"
          ) {
            clearInterval(interval);
            setVisitDetail(updatedDetail);
            setStatusMsg("AI care plan generated successfully!");
            setTimeout(() => router.push("/doctor"), 1500);
          } else if (retries >= maxRetries) {
            clearInterval(interval);
            setStatusMsg("AI care plan is processing in the background. Redirecting to dashboard...");
            setTimeout(() => router.push("/doctor"), 2000);
          }
        } catch (pollErr) {
          console.error("Error polling visit detail status", pollErr);
        }
      }, 2000);
    } catch (err: any) {
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-[#edf4ef] via-[#f8f6f0] to-[#f1f6f2] px-6 py-10 text-[#21322a]">
      <div className="mx-auto max-w-4xl space-y-6">
        ...
        {isGeneratingSummary ? (
          <Card className="rounded-3xl border border-[#3e6b63]/30 bg-gradient-to-br from-[#edf4ef] to-[#f8f6f0] p-8 shadow-sm">
            <CardBody className="space-y-4 text-center py-10 flex flex-col items-center justify-center animate-pulse">
              <span className="text-5xl animate-bounce">🤖</span>
              <h2 className="text-xl font-black text-[#21322a] mt-4">Generating AI care plan...</h2>
              <p className="text-xs text-[#587066] max-w-sm mx-auto leading-relaxed">
                We are generating a patient-friendly summary of the diagnosis, prescriptions, follow-up instructions, and clinical warning flags.
              </p>
